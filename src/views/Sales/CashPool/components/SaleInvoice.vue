...
<div class="app-container">
    <el-card class="box-card">
        <div slot="header" class="clearfix">
            <el-button :disabled="EnableSave" style="float: left" type='success' icon="fa fa-save" @click="createData('tempForm')">{{ $t("CashPool.Save") }}</el-button>
            <span class="demonstration">{{ $t("NewPurchaseInvoice.Box") }}</span>
            <el-select disabled v-model="CashAccount" default-first-option filterable placeholder="صندوق" autocomplete="off">
                <el-option v-for="item in CashAccounts" :key="item.value" :label="item.label" :value="item.value">
            ...
            <el-select disabled v-model="InComeAccount" filterable placeholder="إيراد" autocomplete="off">
                <el-option v-for="item in InComeAccounts" :key="item.value" :label="item.label" :value="item.value">
                    <span style="float: right">{{ item.label }}</span>
                    <span style="float: left color: #8492a6 font-size: 12px">{{
              item.value
            }}</span>
        ...
        <el-table height="250" :data="ItemsMovements" fit border highlight-current-row>
            ...
            <el-table-column align="center">
                <template slot="header" slot-scope="{}">
                    ...
                    <el-button :disabled="EnableSave" style="float: left" icon="el-icon-printer" type='success' @click="printAllItemSale(ItemsMovements)"></el-button>
                </template>
                <template slot-scope="scope">{{
    ...
    <el-card class="box-card">
        <el-table height="250" v-loading="loading" :data="tableData" fit border highlight-current-row ref="multipleTable" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="FakeDate" v-bind:label="$t('CashPool.Date')" width="120" align="center"></el-table-column>
            ...
            <el-table-column prop="PaymentMethod" v-bind:label="$t('CashPool.Pay')" width="150" align="center"></el-table-column>
            ...
            <el-table-column width="60" align="center">
                ...
                <template slot-scope="scope">
                    <el-button icon="el-icon-printer" type="primary" @click="printInvoice(scope.row)"></el-button>
                    <el-button icon="el-icon-printer" type='success' @click="printInvoice1(scope.row)"></el-button>
                </template>
            </el-table-column>
