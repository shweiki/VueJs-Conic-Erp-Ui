      ...
      <el-col :span="5">
        <el-select v-model="operation" filterable placeholder="العملية" @change="setOperations">
          <el-option
            ...
            :key="item.Id"
            :style="'background-color:' + item.color + (item.Id == '0' ? ';' : '20;') + 'margin: 4px; border-radius: 8px;'"
            :label="item.Name"
            :value="item.Id"
            class="options"
          >
            <span
              style="display: flex; color: #8492a6; font-size: 16px; width:100%; justify-content: space-between; align-items: center;"
            >
              <i :style="'color:' + (item.Id == 0 ? '#656b74;' : item.color + ';')" :class="item.icon" /> {{ item.Name
              }}</span>
          </el-option>
        </el-select>
      </el-col>
        ...
        <el-card shadow="always">
          <el-timeline :reverse="reverse" style="height: 350px; overflow: scroll; width: 90%">
            <el-timeline-item
              ...
              :hide-timestamp="true"
            ><span
               :style="item.Type == 'In' ? 'color: #66BB6A' : 'color:#7986cb'"
             >تسجيل {{ item.Type == 'In' ? 'دخول' :
               'خروج' }}</span>
