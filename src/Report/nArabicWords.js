...
const TableScalesP = ['', 'آلاف', 'ملايين', 'مليارات'] // Do not change this table
const TableMale = [
  '',
  'واحد',
  'اثنان',
...
export function nArabicWords(
  ...
  const IsAG = AG === ON // Option Accusative or Genitive case Grammar?
  const SpWa = ' و' // AND word
  const TanweenLetter = 'ًا' // Tanween Fatih for Scale Names above 10
  const Ahad = 'أحد'
  const Ehda = 'إحدى' // Masculine/Feminine 11
  // ---- Setup constants for the AG Option (Accusative/Genitive or Nominative case Grammar)
  const Taa = IsAG ? 'تي' : 'تا'
  const Taan = IsAG ? 'تين' : 'تان' // Hundred 2's مئتا/مائتا مئتان/مائتان
  const Aa = IsAG ? 'ي' : 'ا'
  const Aan = IsAG ? 'ين' : 'ان' // Scale 2's الفا/مليونا الفان/مليونان
  const Ethna = IsAG ? 'اثني' : 'اثنا'
  const Ethnata = IsAG ? 'اثنتي' : 'اثنتا' // Masculine/Feminine 12 starting word
  const Ethnan = IsAG ? 'اثنين' : 'اثنان'
  const Ethnatan = IsAG ? 'اثنتين' : 'اثنتان' // Masculine/Feminine 2
  const Woon = IsAG ? 'ين' : 'ون' // Second part of 20's to 90's
  const IsSubject = Array.isArray(Subject) && Subject.length === 4 // Check for Subject Array Names
  ...
  if (IsSubject) {
    ...
    const space = !IsLastEffTriplet ? '' : ' ' // Position correct spacing
  ...
  function oneTripletToWords() {
    ...
    const Num_100 = ~~(Triplet / 100) // Hundreds (1 digit)
    const Num_Unit = Num_99 % 10 // 0 to 9 (1 digit)
    const Num_Tens = ~~(Num_99 / 10) // Tens   (1 digit)
    let Word_100 = ''
    let Word_99 = '' // Holds words for Hundreds & 0-99
    ...
    if (Num_100) {
      ...
      if (Num_100 > 2) { Word_100 = TableFemale[Num_100] + (SplitHund === ON ? ' ' : '') + Miah }
      // 300-900
      else if (Num_100 === 1) Word_100 = Miah
      // 100
      else {
        Word_100 =
          Miah.slice(0, -1) + ((Scale && !Num_99) || TextToFollow ? Taa : Taan)
      } // 200 Use either مئتا or مئتان
    }
    ...
    if (Scale) {
      ...
      const legalTxt = Legal === ON && Num_99 < 3 ? ' ' + Scale : '' // if Legal Option add Extra Word
      const Word_100Wa = (Num_100 ? Word_100 + legalTxt + SpWa : '') + Scale // Default Scale Name
      if (Num_99 > 2) {
        Words999 +=
